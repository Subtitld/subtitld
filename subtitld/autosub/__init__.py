...
class FLACConverter(object): # pylint: disable=too-few-public-methods
    ...
    """
    def __init__(self, source_path, include_before=0.25, include_after=0.25, ffmpeg_executable='ffmpeg'):
    ...
    def __call__(self, region):
        try:
            ...
            command = [self.ffmpeg_executable, "-ss", str(start), "-t", str(end - start),
...
def extract_audio(filename, channels=1, rate=16000, ffmpeg_executable='ffmpeg'):
    """
...
def generate_subtitles(
    ...
    converter = FLACConverter(source_path=audio_filename, ffmpeg_executable=ffmpeg_executable)
